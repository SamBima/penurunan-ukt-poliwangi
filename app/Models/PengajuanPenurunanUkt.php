...
class PengajuanPenurunanUkt extends Model
{
    ...
    public function getPoinJumlahMotorAttribute()
    {
        ...
        if ($motor === 0) return 0;
        if ($motor === 1) return 5;
        if ($motor === 2) return 10;
        if ($motor === 3) return 15;
        if ($motor === 4) return 20;
        if ($motor === 5) return 25;
        if ($motor === 6) return 30;
        if ($motor === 7) return 35;
        return 40;
    }
    ...
    public function getPoinJumlahMobilAttribute()
    {
        ...
        if ($mobil === 0) return 0;
        if ($mobil === 1) return 5;
        if ($mobil === 2) return 10;
        if ($mobil === 3) return 15;
        if ($mobil === 4) return 20;
        if ($mobil === 5) return 25;
        if ($mobil === 6) return 30;
        if ($mobil === 7) return 35;
        return 40;
    }
