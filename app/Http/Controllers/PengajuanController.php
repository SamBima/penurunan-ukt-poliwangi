...
class PengajuanController extends Controller
{
        ...
        try {
            ...
            HasilValidasi::updateOrCreate(
                ...
                [
                    ...
                    'hasil_wawancara' => 'Pengajuan ditolak oleh Keuangan',
                    'hasil_score' => 0,
                    'rekomendasi_ukt' => $pengajuan->mahasiswa->ukt_awal ?? 0,
                    'status' => 'disarankan_cicilan',
                    'berlaku_selama' => null,
                ]
            );

            return redirect()->back()->with('success', 'Pengajuan berhasil ditolak dengan alasan yang telah disimpan.');
        } catch (\Exception $e) {
            return redirect()->back()->with('error', 'Terjadi kesalahan: ' . $e->getMessage());
        }
    }
        ...
        try {
            ...
            foreach ($pengajuans as $pengajuan) {
                if ($action === 'terima') {
                    ...
                    $sawCriteria = [
                        ['label'=>'Penghasilan Orang Tua', 'nilai'=>(float)$pengajuan->total_gaji,          'min'=>$minGaji,       'max'=>$maxGaji,       'bobot'=>0.20, 'tipe'=>'cost'],
    ...
    private function calculateSawScoresAndRanks($query, $sortOption = 'saw_desc')
    {
        ...
        foreach ($allResults as $item) {
            ...
            $sawCriteria = [
                ['nilai'=>(float)$item->total_gaji,          'min'=>$minGaji,       'max'=>$maxGaji,       'bobot'=>0.20, 'tipe'=>'cost'],
    ...
    public function riwayat(Request $request)
    {
        ...
        if (!$mahasiswa) {
            return view('dashboard.riwayat_pengajuan', ['pengajuans' => collect()->paginate(5)]);
        }
